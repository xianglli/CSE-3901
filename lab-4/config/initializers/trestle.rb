Trestle.configure do |config|
  ...
  #
  config.site_title = "CSE TA application"
  ...
  #
  config.footer = "Powered by Future Tech Corp, CSE 3901"
  ...
  config.auth.avatar = ->(user) {
    avatar(fallback: user.initials) do
      image_tag(user.avator, alt: user.display_name) if user.avator?
    end
  }
  ...
  config.auth.format_user_name = ->(user) {
    content_tag(:strong, user.display_name)
  }
